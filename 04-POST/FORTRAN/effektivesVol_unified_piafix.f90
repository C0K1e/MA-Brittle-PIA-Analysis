...
! v19.0 PIAFix-Variante (Stand: 2026-05-01) — identisch zu effektivesVol_unified
! AUSSER dass die PIA-Auswertung am Gauss-Punkt statt an Knoten passiert.
!
! Abhaengigkeiten:
!   - overhead_unified_piafix.f90  (MODULE overhead_unified_piafix)
...
!
! v19.0 PIAFix Aenderung gegenueber Legacy (siehe Module-Header fuer Details):
!   PIA wird jetzt LOKAL am Gauss-Punkt ausgewertet, nicht mehr ueber
!   Knoten-Aggregation+Interpolation. Tensile cutoff via MAX(sigma_i, 0).
!
! Output-Files (Format identisch zu Legacy):
...
! v22.0: GP_DEBUG-Flag (analog zu AVGModeRAW/GaussPNorm — Float-Lesepuffer + Integer-NINT)
integer :: gp_debug_int
real :: gp_debug_real
...
! v22.0: GP_DEBUG-Flag (optional, rueckwaertskompatibel via IOSTAT-Default).
! Label-Layout in effVol_Parameter.out: 'GPDebug:   ' (11 Zeichen) + F14.0 Wert.
! Python schreibt diese Zeile in _run_gauss_raw_fortran() bei gp_debug=True;
! Append am Ende der Datei. Bei alten Param-Files (vor v22.0) fehlt die Zeile —
! IOSTAT/=0 -> Default 0 -> kein Dump.
READ(101,'(11X,F14.0)', IOSTAT=io_stat) gp_debug_real
IF (io_stat /= 0) gp_debug_real = 0.0
gp_debug_int = NINT(gp_debug_real)
CLOSE(101)

! RAWNodes: Modul-Variablen setzen
stress_mode = stress_mode_int
do_gaussnorm = (gaussnorm_int == 1)
! v22.0: GP_DEBUG-Flag fuer den Dump-Block in veff_gausz/seff_gausz
do_gp_debug = (gp_debug_int == 1)

ausgabename = ADJUSTL(ausgabename)

! DEBUGGING
! v16.1 (A8): Klartext-Konversion fuer stress_mode + do_gaussnorm
! v19.0 PIAFix: Banner-Print zur klaren Unterscheidung im Log
WRITE(*,'(A60)') '============================================================'
WRITE(*,'(A60)') 'EFFEKTIVES VOLUMEN — UNIFIED + PIAFix Variante (v19.0)'
WRITE(*,'(A60)') 'PIA at Gauss-Point: AKTIV (lokale Auswertung statt Knoten)'
WRITE(*,'(A60)') '============================================================'
...
END IF
IF (do_gp_debug) THEN
  WRITE(*,'(A60)') 'GP_DEBUG  = ON (Dump interpolierter GP-Werte fuer Validierung)'
ELSE
  WRITE(*,'(A30)') 'GP_DEBUG  = OFF'
END IF
...
IF (stress_mode == 0) THEN
  ...
  ! v20.2: Pre-Interpolation-Clipping ENTFERNT. Tensile cutoff passiert erst
  ! AM Gauss-Punkt in overhead_unified_piafix (MAX(sigma_i, 0)^m). Pre-Clipping
  ! waere eine nichtlineare Operation vor Interpolation -> Bug-Klasse wie der
  ! urspruengliche PIA-an-Knoten-Bug. interpolate(max(sigma,0)) != max(interpolate(sigma),0).
  ! Ausschliesslich Normieren (linear, vorzeichen-erhaltend).
  PRINT *, '........................................ Check'
  PRINT *, 'Scaling stresses (NOD, no pre-clipping in PIAFix)'
...
ELSE
  ...
  ! v20.2: Pre-Interpolation-Clipping ENTFERNT (siehe NOD-Modus oben fuer Begruendung).
  ! Ausschliesslich Normieren (linear).
  PRINT *, '........................................ Check'
  PRINT *, 'Scaling stresses (RAW, no pre-clipping in PIAFix)'
...
! v22.0: smax_file an Modul weitergeben — der GP_DEBUG-Dump in veff_gausz/seff_gausz
! multipliziert die normalisierten sigmaN-Werte damit zurueck zu physikalischen Werten,
! sodass Python direkt mit -S-GP-clean.csv (gleiche Einheit) vergleichen kann.
smax_file_module = smax_file
